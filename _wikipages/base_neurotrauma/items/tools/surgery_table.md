...
blocks:
  - type: description
    ...
    text: |
     ...
     - {{ANALGESIC_TANK}}
     - {{DIALYSIS_FILTER}} or {{TOXIN_FILTER}}
     - {{REFRIGERATED_CRATE}}
     ...
     An unconscious patient can be forced to lay on the table when grabbing them and using the Select key (default left click). However, patients cannot be grabbed when laying on the table, and must be removed from the table using Select key before attempting to access their Health UI.
